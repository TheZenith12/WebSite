...
function Hero({ searchTerm, setSearchTerm, list = [], activeCategory, setActiveCategory }) {
  ...
  const counts = {
    ...
    uzseglent_gazar: list.filter((r) => r.category === "uzseglent_gazar").length,
  };
              ...
              <div className={`relative bg-white rounded-2xl md:rounded-3xl shadow-2xl transition-all duration-300 ${isFocused ? 'ring-2 ring-teal-300' : ''}`}>
                <div className="flex items-center gap-2 p-1.5 md:p-2">
                  <div className="flex items-center justify-center w-10 h-10 md:w-14 md:h-14 bg-gradient-to-br from-teal-500 to-emerald-500 rounded-xl md:rounded-2xl ml-1 shadow-lg flex-shrink-0">
                    <svg className={`w-5 h-5 md:w-6 md:h-6 text-white transition-transform duration-500 ${isFocused ? 'scale-110' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  />
                  {searchTerm && (
                    <button onClick={() => setSearchTerm('')} className="flex items-center justify-center w-8 h-8 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all flex-shrink-0">
                ...
                {isFocused && !searchTerm && (
                  <div className="px-4 md:px-6 pb-3 md:pb-4 flex flex-wrap gap-2 animate-fadeIn">
                    {['Төв аймаг', 'Архангай', 'Өвөрхангай', 'Хэнтий'].map((s, i) => (
                      <button key={i} onMouseDown={() => setSearchTerm(s)} className="px-3 py-1.5 bg-teal-50 hover:bg-teal-100 text-teal-700 rounded-full text-xs md:text-sm font-semibold transition-all hover:scale-105 shadow-sm">
                        {s}
                      </button>
                    ))}
                  </div>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <span className="text-3xl md:text-5xl leading-none">{cat.icon}</span>
                  <span className="text-white font-bold text-xs md:text-base leading-tight text-center">
                    {cat.label}
                  </span>
                  <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full transition-all ${isActive ? 'bg-white/25 text-white' : 'bg-white/15 text-white/70 group-hover:bg-white/20'}`}>
                    {counts[cat.key] || 0} газар
                  </span>

                  {/* Animated arrow when active */}
                  <div className={`transition-all duration-300 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'}`}>
